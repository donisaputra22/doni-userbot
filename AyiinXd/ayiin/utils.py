...
async def autopilot():
    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="Doni - 𝚄𝚜𝚎𝚛𝚋𝚘𝚝 Lᴏɢs",
                about="» Group log Created by: Doni-userbot\n\n» Support : @Donisupport\n» Support: @supportNande",
                megagroup=True,
            ),
        )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/2ba9d146ff35f6e6eafd6.jpg", "photoyins.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"Doni{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    filogo = random.choice(
          [
              "https://telegra.ph/file/2ba9d146ff35f6e6eafd6.jpg",
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Doni{(str(who.id))[6:]}{str(ran)}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
